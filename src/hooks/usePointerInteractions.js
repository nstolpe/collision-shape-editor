...
import { closestPointOnSegment, translation, withinAABB } from 'Utility/math';
...
const selector = ({
  ...
  vertices: shapes.reduce((vertices, shape, _, shapeKey) => {
    shape.vertices.forEach((vertex, _, vertexKey) => {
      const key = `${addPrefix(
        VERTEX,
        vertexKey
...
const usePointerInteraction = () => {
  ...
  const isVertexSelected = ({ name }) =>
    !!selectedVertices.find((vertex) => vertex.name === name);
  ...
  const areVerticesSelected = (checkVertices) =>
    checkVertices.reduce((areSelected, vertex) => {
      const { name } = vertex;
  ...
  const updateTranslations = (coordinates) => {
    Object.keys(selectedVertices).forEach((name) => {
      ...
      if (x && y) {
        ...
        console.log(
          "@TODO: if you never see this, remove it. If you do, figure out why you're seeing it"
        );
      }
    });
  ...
  const selectEdge = (coordinates, name) => {
    ...
    const targetEdgeVertices = [vertexId1, vertexId2].reduce(
      (newVertices, id) => {
    ...
    switch (true) {
      ...
      default: {
        const vertex1Selected = selectedVertices.hasOwnProperty(
          addPrefix(
            VERTEX,
            `${vertexId1}${DEFAULT_DELIMITER}${name.substring(
              name.indexOf(SHAPE)
            )}`
          )
        );
        const vertex2Selected = selectedVertices.hasOwnProperty(
          addPrefix(
            VERTEX,
            `${vertexId2}${DEFAULT_DELIMITER}${name.substring(
              name.indexOf(SHAPE)
            )}`
          )
        );
        if (!vertex1Selected || !vertex2Selected) {
          dispatch(
            setSelectedVertices(
              targetEdgeVertices.reduce((vertices, { distance, name }) => {
    ...
    if (button === 2) {
      ...
      switch (true) {
        case target.name === 'VIEWPORT': {
          ...
          const [targetShape, targetShapeKey] =
            shapes.reduce((result, shape, index, key) => {
          ...
          if (targetShapeKey) {
            dispatch(
              setContextMenu(SHAPE, clientX, clientY, {
                shape: targetShape,
  ...
  const handlePointerMove = (event) => {
    ...
    const pointerIndex = activePointers.findIndex(
  ...
  const extendShapeOrAddShape = (coordinates) => {
    ...
    if (selectedVertexKeys.length === 1) {
      ...
      if (!shape.closed && vertexKey === vertices.keys[0]) {
        ...
        return dispatch(
          insertVertexBefore({ shapeKey, vertexKey, x, y, makeSelected: true })
        );
      } else if (
        !shape.closed &&
        ...
        return dispatch(
          insertVertexAfter({ shapeKey, vertexKey, x, y, makeSelected: true })
        );
      }
    }
  ...
  const pointerDownVertex = (event, coordinates) => {
    const { data: { identifier } = {}, target: { name, position } = {} } =
      event;
  ...
  const pointerDownEdge = (event, coordinates) => {
    const { data: { identifier } = {}, target: { name } = {} } = event;
  ...
  const pointerMoveVertex = (event, activePointer, pointerCoordinates) => {
    const { data: { originalEvent: { shiftKey = false } = {} } = {} } = event;
  ...
  const pointerMoveEdge = (event, pointer, coordinates) => {
    ...
    if (Object.keys(selectedVertices).length && !justRemoved) {
      ...
      dispatch(
        setVertexPositionsRelativeToCoordinates(selectedVertices, coordinates)
      );
    }
  };
  ...
  const pointerUpViewport = () => {
    ...
    if (selectOverlay.enabled) {
      ...
      if (addModifierKeyPressed) {
        ...
        addSelectedVertices(
          targetVertices.reduce((newSelectedVertices, { x, y }, idx, key) => {
  ...
  const pointerUpVertex = (event) => {
    const {
      data: { identifier, originalEvent: { shiftKey = false } = {} } = {},
    } = event;
