 ...
 * @return {string}
 */
export const rgbToHexString = ({ r, g, b }, hex = true) =>
  `${hex ? '#' : ''}${((1 << 24) + (r << 16) + (g << 8) + b)
    .toString(16)
    .slice(1)}`;
export const rgbToNum = ({ r, g, b }) =>
  parseInt(rgbToHexString({ r, g, b }, false), 16);
...
export const numToHexString = (number, hex = true, min = 6) => {
  const matches = (parseInt(number, 10) || Object(number))
    .toString(16)
    .split('.')[0]
    .match(/^(?:#|0x)?(?<hex>[0-9A-Fa-f]+)$/);
  const hexString = parseInt(
    property(matches, ['groups', 'hex'], 0),
    16
  ).toString(16);

  return `${hex ? '#' : ''}${
    '0'.repeat(Math.max(min - hexString.length, 0)) + hexString
  }`;
};
